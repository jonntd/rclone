#!/bin/bash
# 跨网盘复制测试脚本: 123网盘 → 115网盘
# 版本: 1.0
# 用途: 测试123网盘到115网盘的跨平台数据传输能力
...
# 测试配置
SOURCE_DIR=${SOURCE_DIR:-"123_test_unified"}      # 123网盘源目录
TARGET_DIR=${TARGET_DIR:-"115_test_unified"}      # 115网盘目标目录
...
MAIN_LOG="$LOG_DIR/123_to_115_main.log"
COPY_LOG="$LOG_DIR/123_to_115_copy.log"
VERIFY_LOG="$LOG_DIR/123_to_115_verify.log"
...
cleanup() {
    ...
    if [ "$CLEAN_TARGET" = "true" ]; then
        log_info "清理目标目录: 115:$TARGET_DIR/"
        if ./rclone_test ls "115:$TARGET_DIR/" >/dev/null 2>&1; then
            ./rclone_test delete "115:$TARGET_DIR/" 2>/dev/null || true
            log_info "目标目录清理完成"
        else
...
# 执行跨网盘复制
perform_cross_copy() {
    local source="123:$SOURCE_DIR/"
    local target="115:$TARGET_DIR/"
    
    log_info "开始跨网盘复制: $source → $target"
    
    # 构建rclone命令 - 针对115网盘的特殊配置
    ...
    rclone_cmd="$rclone_cmd --ignore-size"  # 忽略大小检查，强制重新上传
    rclone_cmd="$rclone_cmd --ignore-times"  # 忽略时间检查
...
main() {
    ...
    log_info "跨网盘复制测试开始"
    log_info "源目录: 123:$SOURCE_DIR/"
    log_info "目标目录: 115:$TARGET_DIR/"
    ...
    # 步骤2: 检查源目录
    log_info "步骤2: 检查源目录"
    if ! ./rclone_test ls "123:$SOURCE_DIR/" >/dev/null 2>&1; then
        log_error "源目录不存在: 123:$SOURCE_DIR/"
        log_info "请先运行 123网盘测试脚本生成测试文件"
        exit 1
    fi
    log_success "源目录存在: 123:$SOURCE_DIR/"
    
    # 获取源目录信息
    local source_info="$LOG_DIR/source_info.json"
    get_directory_info "123:$SOURCE_DIR" "$source_info" || exit 1
    
    # 步骤3: 准备目标目录
    log_info "步骤3: 准备目标目录"
    ./rclone_test mkdir "115:$TARGET_DIR/" 2>/dev/null || true
    log_success "目标目录准备完成: 115:$TARGET_DIR/"
    ...
    if [ "$VERIFY_INTEGRITY" = "true" ]; then
        ...
        get_directory_info "115:$TARGET_DIR" "$target_info" || exit 1
...
quick_test() {
    ...
    # 连接验证
    if ! ./rclone_test about 123: >/dev/null 2>&1 || ! ./rclone_test about 115: >/dev/null 2>&1; then
        log_error "网盘连接失败"
        exit 1
    fi
    
    # 快速复制测试（只复制小文件）- 针对115网盘的特殊配置
    log_info "执行快速复制测试"
    if ./rclone_test copy "123:$SOURCE_DIR/" "115:$TARGET_DIR/" --include "*small*" --ignore-size --ignore-times -vv --log-file "$COPY_LOG"; then
        echo "✅ 快速复制测试通过！"
        exit 0
    else
        echo "❌ 快速复制测试失败！"
...
环境变量配置:
  SOURCE_DIR          123网盘源目录名, 默认: 123_test_unified
  TARGET_DIR          115网盘目标目录名, 默认: 115_test_unified
